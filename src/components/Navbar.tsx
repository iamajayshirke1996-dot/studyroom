...
export const Navbar: React.FC = () => {
  ...
  const moduleItems = [
      ...
      ? [
          {
            id: 'jobs',
            label: 'Job Tracker',
            description: 'Applications, outreach & interviews',
            icon: Briefcase,
            badge: 'Career',
      ...
      ? [
          {
            id: 'shorts',
            label: 'Shorts 🎬',
            description: 'Content creation kanban & metrics',
            icon: Video,
            badge: 'Content',
      ...
      ? [
          {
            id: 'maang',
            label: 'MAANG Prep',
            description: '12-Week System Design & Coding Roadmap',
            icon: Flame,
            badge: 'Roadmap',
      ...
      : []),
    {
      id: 'summary',
      label: 'Summaries',
      description: 'AI study session notes & insights',
      icon: Brain,
      badge: 'AI',
  ...
  const activeModuleItem = moduleItems.find((m) => m.id === activeTab);
  const isModuleActive = Boolean(activeModuleItem);
  ...
  const showAdminTab = currentUserPermissions?.isAdmin || isAdminEmail(user?.email);

  const mobileNavItems = [
    ...primaryNavItems,
    ...moduleItems,
    ...(showAdminTab ? [{ id: 'admin', label: 'Admin ⚙️', icon: ShieldCheck }] : []),
  ];
    ...
    <header className="sticky top-0 z-40 w-full border-b border-slate-200 dark:border-slate-800/80 bg-white/95 dark:bg-slate-950/90 backdrop-blur-md transition-colors">
      <div className="max-w-7xl mx-auto px-3 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16 gap-2">
          ...
          {/* Center: Clean Primary Tabs + Popover Submenu */}
          <nav className="hidden md:flex items-center gap-1 bg-slate-100 dark:bg-slate-900 p-1 rounded-xl border border-slate-200 dark:border-slate-800 shrink-0">
            ...
            {/* Modules Popover Sub-Menu Dropdown */}
            {moduleItems.length > 0 && (
              <div className="relative" ref={modulesMenuRef}>
                <button
                  ...
                  className={`flex items-center gap-1.5 px-3.5 py-1.5 rounded-lg text-xs font-semibold whitespace-nowrap transition-all ${
                    isModuleActive
                      ? 'bg-white dark:bg-slate-800 text-indigo-600 dark:text-indigo-400 shadow-xs font-bold ring-1 ring-indigo-500/30'
                      : 'text-slate-600 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white'
                  }`}
                >
                  {activeModuleItem ? (
                    <activeModuleItem.icon className="h-3.5 w-3.5 text-indigo-500" />
                  ) : (
                    <Layers className="h-3.5 w-3.5 text-indigo-500" />
                  )}
                  <span>{activeModuleItem ? activeModuleItem.label : 'Modules'}</span>
                  <ChevronDown
                    className={`h-3 w-3 transition-transform duration-200 ${
                      isModulesMenuOpen ? 'rotate-180' : ''
                    }`}
                  />
                </button>
                ...
                {isModulesMenuOpen && (
                  <div className="absolute left-0 mt-2 w-72 rounded-2xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 shadow-2xl p-2 z-50 animate-in fade-in zoom-in-95 duration-150">
                          ...
                          >
                            ...
                            <div className="flex-1 min-w-0">
                              <div className="flex items-center justify-between">
                                ...
                                <span
                                  className={`text-[9px] font-semibold px-1.5 py-0.5 rounded-full border ${mod.badgeColor}`}
                                >
                                  {mod.badge}
                                </span>
                              </div>
