...
import { BookOpen, ExternalLink, Brain, Clock, MoreVertical, Plus, CheckCircle2, User, Edit3, Trash2, Share2 } from 'lucide-react';
...
export const UdemyCourseCard: React.FC<Props> = ({ goal }) => {
  const {
    incrementProgress,
  ...
  const total = goal.totalLectures || 1;
  const completed = goal.completedLectures || 0;
  ...
  return (
    <div className="bg-white dark:bg-slate-900/80 rounded-2xl p-5 border border-slate-200 dark:border-slate-800 hover:border-purple-500/40 shadow-sm transition-all flex flex-col justify-between relative group">
      <div>
        {/* Top Header */}
        <div className="flex items-start justify-between gap-3 mb-3">
          <div className="flex items-center gap-2 flex-wrap">
            <span className="text-[11px] font-bold uppercase tracking-wider px-2 py-0.5 rounded bg-purple-500/15 text-purple-700 dark:text-purple-300 border border-purple-500/30 flex items-center gap-1">
            ...
            {menuOpen && (
              ...
              >
                <button
                  ...
                  className="w-full text-left px-3 py-1.5 text-xs text-[#0A66C2] dark:text-blue-400 font-bold hover:bg-blue-50 dark:hover:bg-blue-950/20 rounded-lg flex items-center gap-2"
                >
        ...
        <div className="space-y-2 mb-4">
          <div className="flex items-center justify-between text-xs">
            <span className="text-slate-500 dark:text-slate-400 font-medium">Lecture Progress</span>
            <span className="font-bold text-slate-900 dark:text-white">
              {completed} / {total} <span className="text-purple-600 dark:text-purple-400 font-normal">({percent}%)</span>
            </span>
          </div>
          ...
          {goal.totalSections && (
            <div className="flex justify-between text-[11px] text-slate-500 dark:text-slate-400 pt-0.5">
              <span>Sections: {goal.completedSections || 0} of {goal.totalSections}</span>
      ...
      <div className="pt-3 border-t border-slate-100 dark:border-slate-800/80 flex items-center gap-2">
        {/* +1 Lecture Quick Action */}
        <button
          onClick={() => incrementProgress(goal.id, 1)}
          className="flex-1 py-2 px-3 rounded-xl bg-purple-500/10 hover:bg-purple-500/20 text-purple-700 dark:text-purple-300 border border-purple-500/20 text-xs font-bold transition-all flex items-center justify-center gap-1.5 shadow-sm"
          title="Increment +1 completed lecture"
        >
